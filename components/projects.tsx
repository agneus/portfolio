...
export function Projects() {
  const projects = [
    ...
    },
    {
      ...
      tags: ["React Native", "Expo", "TypeScript", "Solana", "Blockchain", "AI"],
      ...
      <div className="container">
        <ScrollGlitchContainer className="text-center mb-16" threshold={0.5}>
          <h2 className="font-mono text-3xl font-bold tracking-tighter mb-4">
            <span className="text-green-500">+</span> <MatrixText text="PROJECTS" className="inline-block" />{" "}
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {projects.map((project, index) => (
            <CodeBuildEffect key={project.id} delay={index * 200} lines={8} threshold={0.3}>
              <Card className="bg-black border-gray-800 overflow-hidden group">
                ...
                <CardContent>
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map((tag, tagIndex) => (
                      ...
                      >
                        <Badge className="bg-green-500/20 text-green-500 font-mono text-xs text-scramble">{tag}</Badge>
                      </motion.div>
                    ))}
                  </div>
                </CardContent>
                <CardFooter className="flex justify-between">
                  ...
                  <a href={project.liveUrl} target="_blank" rel="noopener noreferrer">
                    <HackerButton size="sm" className="font-mono text-xs bg-green-500 text-black hover:bg-green-600">
                      Live Demo
                    </HackerButton>
                  </a>
                </CardFooter>
